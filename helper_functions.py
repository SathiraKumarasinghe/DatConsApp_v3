...
def app_meta(icon):
    ...
    # Set website details
    st.set_page_config(page_title ="DataCon", 
                       page_icon=icon, 
                       layout='centered')
...
def display_app_header(main_txt,sub_txt,is_sidebar = False):
    ...
    html_temp = f"""
    <h2 style = "color:#F2EDEC; text_align:center; font-weight: bold;"> {main_txt} </h2>
    <p style = "color:#F2EDEC; text_align:center;"> {sub_txt} </p>
    </div>
    """
    if is_sidebar:
...
def sub_text(text):
    ...
    html_temp = f"""
    <p style = "color:#F2EDEC; text_align:justify;"> {text} </p>
    </div>
    """
